...
def load_config():
    ...
    return {
        ...
        "UI": {
            ...
            # New: explicit webcam rotation setting used by camera module
            "webcam_rotation": config.getint('UI', 'webcam_rotation', fallback=config.getint('UI', 'rotation', fallback=0)),
            ...
            # Optional: OpenCV camera device index (ignored when use_camera_module=True)
            "camera_device_index": config.getint('UI', 'camera_device_index', fallback=0),       
        },
        "BATTERY": {
