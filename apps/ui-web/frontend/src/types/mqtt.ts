...
export function isSTTFinalMessage(payload: unknown): payload is STTFinalMessage {
  return (
    ...
    payload !== null &&
    'text' in payload &&
    typeof (payload as STTFinalMessage).text === 'string' &&
    (payload as STTFinalMessage).is_final === true
  )
}
...
export function isSTTPartialMessage(payload: unknown): payload is STTPartialMessage {
  return (
    ...
    payload !== null &&
    'text' in payload &&
    typeof (payload as STTPartialMessage).text === 'string' &&
    (payload as STTPartialMessage).is_final === false
  )
}
...
export function isLLMStreamMessage(payload: unknown): payload is LLMStreamMessage {
  return (
    ...
    payload !== null &&
    'data' in payload &&
    typeof (payload as LLMStreamMessage).data === 'object' &&
    'delta' in (payload as LLMStreamMessage).data &&
    'id' in (payload as LLMStreamMessage).data
  )
}
...
export function isLLMResponseMessage(payload: unknown): payload is LLMResponseMessage {
  return (
    ...
    payload !== null &&
    'data' in payload &&
    typeof (payload as LLMResponseMessage).data === 'object' &&
    'id' in (payload as LLMResponseMessage).data &&
    ('reply' in (payload as LLMResponseMessage).data ||
      'error' in (payload as LLMResponseMessage).data)
  )
}
...
export function isTTSSayMessage(payload: unknown): payload is TTSSayMessage {
  return (
    ...
    payload !== null &&
    'text' in payload &&
    typeof (payload as TTSSayMessage).text === 'string'
  )
}
...
export function isTTSStatusMessage(payload: unknown): payload is TTSStatusMessage {
  return (
    ...
    payload !== null &&
    'event' in payload &&
    typeof (payload as TTSStatusMessage).event === 'string' &&
    ['speaking_start', 'speaking_end'].includes((payload as TTSStatusMessage).event)
  )
}
...
export function isMemoryResultsMessage(payload: unknown): payload is MemoryResultsMessage {
  return (
    ...
    payload !== null &&
    'results' in payload &&
    Array.isArray((payload as MemoryResultsMessage).results)
  )
}
...
export function isAudioFFTMessage(payload: unknown): payload is AudioFFTMessage {
  return (
    ...
    payload !== null &&
    'fft' in payload &&
    Array.isArray((payload as AudioFFTMessage).fft)
  )
}
...
export function isHealthMessage(payload: unknown): payload is HealthMessage {
  ...
  if ('data' in payload && typeof (payload as HealthMessage).data === 'object') {
    const data = (payload as HealthMessage).data
    return data !== undefined && 'ok' in data && typeof data.ok === 'boolean'
  }

  // Check for unwrapped format
  return 'ok' in payload && typeof (payload as HealthMessage).ok === 'boolean'
}
