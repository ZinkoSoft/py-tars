...
class AudioFanoutPublisher:
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            await reader.read()  # keep coroutine alive until client disconnects
        except Exception:
            pass
        finally:
            ...
            with contextlib.suppress(Exception):
                await writer.wait_closed()
            _LOG.info("Audio fan-out client disconnected: %s", peer)
    ...
    def _interp(self, chunk: bytes, sample_rate: int) -> bytes:
        ...
        # Keep last few source samples for continuity
