...
class RAGHandler:
    ...
    async def query(
        ...
        if use_cache:
            cache_key = self._generate_cache_key(prompt, top_k, max_tokens, include_context, context_window, retrieval_strategy)
            ...
            if cached_result is not None:
                ...
                logger.info(
                    ...
                    latency, cached_result.token_count, cache_key[:12]
                )
        ...
        try:
            ...
            logger.info(
                ...
                latency, context.token_count, context.truncated, context.strategy_used
            )
            ...
            latency = time.monotonic() - start_time
            logger.warning(
                "RAG query timeout: correlation_id=%s, latency=%.3fs",
    ...
    def _generate_cache_key(
        ...
        cache_input = f"{prompt}|{top_k}|{max_tokens}|{include_context}|{context_window}|{retrieval_strategy}"
    ...
    def _add_to_cache(self, cache_key: str, context: RAGContext) -> None:
        ...
        logger.debug(
            ...
            cache_key[:12], context.token_count, len(self._cache)
        )
