...
class _ModelRouter:
    def __init__(self, responses_patterns: Sequence[str] | None = None):
        self._responses_patterns = tuple(pattern.strip() for pattern in (responses_patterns or ()) if pattern.strip())
...
def _collect_output_content(item: dict[str, Any], text_parts: list[str], tool_calls: list[dict[str, Any]]) -> None:
...
class OpenAIProvider(LLMProvider):
    ...
    async def stream_chat(self, messages: list[dict[str, Any]], **kwargs) -> AsyncIterator[dict[str, str | None]]:
        model = kwargs.get("model")
        max_tokens = kwargs.get("max_tokens")
        temperature = kwargs.get("temperature")
        top_p = kwargs.get("top_p")
