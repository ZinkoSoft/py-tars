        ...
        *,
        ...
        if topic == tools_registry_topic:
            ...
            try:
                ...
                logger.debug(
                    "Parsed registry data with %d tools", len(registry_data.get("tools", []))
                )
        ...
        if topic == memory_results_topic:
            logger.info(
                "MEMORY RESULTS MESSAGE RECEIVED: topic=%s, payload_size=%d",
                topic,
                len(message.payload),
            )
            try:
                ...
                logger.debug(
                    "Memory results received: envelope_id=%s, data_keys=%s",
                    envelope.id,
                    list(data.keys()),
                )
                ...
                try:
                    ...
                    logger.info(
                        "MEMORY RESULTS FALLBACK PARSING: data_keys=%s",
