...
class MCPToolClient:
    ...
    async def _execute_with_stdio(
        self, server_name: str, command: str, args: list, tool_name: str, arguments: dict
    ) -> dict:
        ...
        try:
            ...
            async with stdio_client(server_params) as (read_stream, write_stream):
                ...
                logger.info(
                    f"stdio_client context entered, streams: read={type(read_stream).__name__}, write={type(write_stream).__name__}"
                )
                async with ClientSession(read_stream, write_stream) as session:
                    logger.info("ClientSession created, calling initialize()...")
                    try:
