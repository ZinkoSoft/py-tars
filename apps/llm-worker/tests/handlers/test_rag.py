...
async def test_query_successful(rag_handler, mock_mqtt_client, mock_client):
    ...
    query_task = asyncio.create_task(
        rag_handler.query(mock_mqtt_client, mock_client, "What is Python?", 5, correlation_id, use_cache=False)
    )
...
async def test_query_timeout(rag_handler, mock_mqtt_client, mock_client):
    ...
    context = await rag_handler.query(mock_mqtt_client, mock_client, "test query", 5, correlation_id, use_cache=False)
...
async def test_query_publishes_correctly(rag_handler, mock_mqtt_client, mock_client):
    ...
    query_task = asyncio.create_task(
        rag_handler.query(mock_mqtt_client, mock_client, "test prompt", 3, correlation_id, use_cache=False)
    )
    await asyncio.sleep(0.05)
    
    # Resolve immediately
    rag_handler.handle_results({"id": correlation_id, "results": [], "total_tokens": 0, "truncated": False, "strategy_used": "hybrid"})
    await query_task
...
async def test_handle_results_already_resolved(rag_handler, mock_mqtt_client, mock_client):
    ...
    # Resolve twice
    rag_handler.handle_results({"id": correlation_id, "results": [], "total_tokens": 0, "truncated": False, "strategy_used": "hybrid"})
    rag_handler.handle_results({"id": correlation_id, "results": [], "total_tokens": 0, "truncated": False, "strategy_used": "hybrid"})  # Second call should be no-op
...
def test_handle_results_empty_results(rag_handler):
    ...
    results_data = {"id": correlation_id, "results": [], "total_tokens": 0, "truncated": False, "strategy_used": "hybrid"}
    rag_handler.handle_results(results_data)
...
async def test_multiple_concurrent_queries(rag_handler, mock_mqtt_client, mock_client):
    ...
    query1 = asyncio.create_task(rag_handler.query(mock_mqtt_client, mock_client, "query1", 5, "corr-1", use_cache=False))
    query2 = asyncio.create_task(rag_handler.query(mock_mqtt_client, mock_client, "query2", 5, "corr-2", use_cache=False))
    query3 = asyncio.create_task(rag_handler.query(mock_mqtt_client, mock_client, "query3", 5, "corr-3", use_cache=False))
    
    await asyncio.sleep(0.05)
    
    # Resolve out of order
    rag_handler.handle_results({"id": "corr-2", "results": [{"document": {"text": "Result 2"}, "context_type": "target"}], "total_tokens": 5, "truncated": False, "strategy_used": "hybrid"})
    rag_handler.handle_results({"id": "corr-1", "results": [{"document": {"text": "Result 1"}, "context_type": "target"}], "total_tokens": 5, "truncated": False, "strategy_used": "hybrid"})
    rag_handler.handle_results({"id": "corr-3", "results": [{"document": {"text": "Result 3"}, "context_type": "target"}], "total_tokens": 5, "truncated": False, "strategy_used": "hybrid"})
...
async def test_query_exception_cleanup(rag_handler, mock_mqtt_client, mock_client):
    ...
    context = await rag_handler.query(mock_mqtt_client, mock_client, "test", 5, correlation_id, use_cache=False)
