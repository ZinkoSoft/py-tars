...
async def test_e2e_config_file_content_validity(tmp_path):
    ...
    for server in config["servers"]:
        required_server_fields = [
            "name", "source", "transport", "command", "args", "env",
            "allowed_tools", "package_name", "installed", "install_path", "url"
        ]
    ...
    required_install_fields = [
        "total_servers", "installed", "already_installed",
        "failed", "skipped", "success_rate", "duration_sec"
    ]
