...
def test_memory_query_roundtrip() -> None:
    ...
    results = MemoryResults(
        ...
        k=2,
        results=[{"document": {"text": "At the lab"}, "score": 0.9}],
    )
    env_out = Envelope.new(
        event_type=EVENT_TYPE_MEMORY_RESULTS,
        data=results,
        correlate=query.message_id,
        source="test-memory",
    )
    ...
    restored_get = CharacterGetRequest.model_validate(
        Envelope.model_validate_json(env_get.model_dump_json()).data
    )
    ...
    restored_snapshot = CharacterSnapshot.model_validate(
        Envelope.model_validate_json(env_snapshot.model_dump_json()).data
    )
    ...
    restored_section = CharacterSection.model_validate(
        Envelope.model_validate_json(env_section.model_dump_json()).data
    )
