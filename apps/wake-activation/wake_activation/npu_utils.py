...
def check_npu_availability() -> Tuple[bool, str]:
    ...
    if rknpu_device.exists():
        ...
        if npu_render_node:
            ...
            available_devices = [str(p) for p in Path("/dev/dri").glob("renderD*")] if Path("/dev/dri").exists() else []
            status_messages.append(f"❌ No NPU device nodes found (/dev/rknpu or renderD129). Available: {available_devices}")
